...
export function mapApiToDept(api: ApiDeptDetail): DepartmentData {
  ...
  return {
    ...
    head: {
      ...
      audience: (head as { audience?: string | null })?.audience ?? undefined,
    ...
    team: [
      ...
      ...(api.team?.map((m) => ({
        name: m.name ?? "",
        role: m.role ?? "",
        specialty: (m as { specialty?: string | null }).specialty ?? "",
        email: m.email ?? undefined,
        audience: (m as { audience?: string | null }).audience ?? undefined,
